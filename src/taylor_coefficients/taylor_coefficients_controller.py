...
class TaylorCoefficientsController:
    ...
    def get_user_input(self):
        ...
        try:
            ...
            if number_of_required_coefficients < 0:
                self.show_error(
                    "Entrada inválida", f"El número de coeficientes requeridos debe ser mayor o igual a 0.")
                return None
        except ValueError:
            self.show_error("Entrada inválida",
                            "Por favor ingrese un número válido.")
        ...
        return xmin, xmax, ymin, ymax, number_of_required_coefficients
    ...
    def update_view(self, taylor_coefficients_model):
        ...
        ymin = taylor_coefficients_model.get_ymin()
        ymax = taylor_coefficients_model.get_ymax()
        ...
        self.plot_taylor_approximation_function(
            taylor_approximation_function, analytic_function, xmin, xmax, ymin, ymax, number_of_required_coefficients)

    def plot_taylor_approximation_function(self, taylor_approximation_function, analytic_function, xmin, xmax, ymin, ymax, number_of_required_coefficients):
        # Generate the data for the charge curve
        # Create an array of x values
        x_data = np.linspace(xmin, xmax, 1000)
        # Calculate the y values using the taylor approximation function
        y_taylor_data = [taylor_approximation_function(x) for x in x_data]
        # Calculate the y values using the analytic function
        ...
        self.view.ax.plot(
            x_data, y_taylor_data, label=fr"$y = T_{{{number_of_required_coefficients-1}}}f(x)$")
    ...
    def submit(self):
        ...
        # Get the user inputs
        xmin, xmax, ymin, ymax, number_of_required_coefficients = inputs

        # Create a new model with the user inputs
        taylor_coefficients_model = TaylorCoefficientsModel(
            xmin, xmax, ymin, ymax, number_of_required_coefficients)
